...
- `--effort`：推理深度 `low` / `medium`(默认) / `high`。`medium`/`high` = **深度模式**：先思考、把简短意图扩写成精细摄影 brief 再画，更真实但慢 ~2–4×，并把扩写后的 brief 存到 `<输出>.brief.txt`；`low` = 快档原样透传。用户**明确要快**时才用 `--effort low`
...
5. 质量默认 `high`、推理 `--effort` 默认 `medium`（深度模式，较慢，约 1–3 分钟，会自动扩写 prompt）；用户**明确要快**就加 `--effort low`（必要时再加 `-q low`）。服务端对上游瞬断/限流会自动重试，慢不等于挂了，**耐心等脚本退出**。
