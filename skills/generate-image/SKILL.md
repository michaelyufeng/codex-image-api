...
- `--effort`：推理深度 `low`(默认) / `medium` / `high`。`low`(默认) = 快档原样透传（prompt 原样发，不扩写）；`medium`/`high` = **深度模式**：先思考、把简短意图扩写成精细摄影 brief 再画，更真实但慢 ~2–4×，并把扩写后的 brief 存到 `<输出>.brief.txt`。用户**明确要更真实/高级感**时才加 `--effort medium` 或 `high`
- `--timeout`：HTTP 超时秒数（默认按 `-n` 和 effort 自动放大，一般不用手动传）
...
5. 质量默认 `high`、推理 `--effort` 默认 `low`（快档原样透传，较快）；用户**明确要更真实/深度扩写**时才加 `--effort medium` 或 `high`（深度模式，较慢，约 1–3 分钟，会自动扩写 prompt）。服务端对上游瞬断/限流会自动重试，慢不等于挂了，**耐心等脚本退出**。
